...
/**
 * Miniaturas editoriais do blog (`<slug-do-post>-blog-premium.webp`) em
 ...
 * substituir o WEBP na pasta da categoria e rodar `bun run build`.
 */
const raw = import.meta.glob<{ default: ImageMetadata }>('../assets/images/blog/**/*-blog-premium.webp', {
	eager: true,
	import: 'default',
});
...
for (const path of Object.keys(raw)) {
	...
	const m = /^(.+)-blog-premium\.webp$/.exec(file);
	if (m?.[1]) {
