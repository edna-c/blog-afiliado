...
const SRC = 'public/images/melhores/melhor-fogao-4-bocas/_src';
const OUT = 'public/images/melhores/melhor-fogao-4-bocas';
const SIZE = 1254;
/** Escala do produto na moldura (~90% do lado maior; antes 0.82). */
...
/** Remove only background connected to image edges (safe for white product bodies). */
async function removeEdgeConnectedBackground(
	...
	// Soft fringe near remaining semi-white edge pixels
	if (soft > 0) {
		for (let y = 0; y < height; y++) {
			for (let x = 0; x < width; x++) {
				const idx = y * width + x;
...
await treat({
	input: path.join(SRC, 'consul-02.jpg'),
	mode: 'white',
	outName: 'consul-cf04nar.webp',
});
await treat({
	input: path.join(SRC, 'electrolux-perspective.jpg'),
	mode: 'white',
	outName: 'electrolux-fe4iw.webp',
});
await treat({
	input: path.join(SRC, 'atlas-preto-3q.png'),
	mode: 'black',
	outName: 'atlas-monaco-plus.webp',
});

for (const [src, dest] of [
	['consul-cf04nar.webp', 'consul-cf04nar-hero.webp'],
	['electrolux-fe4iw.webp', 'electrolux-fe4iw-hero.webp'],
]) {
